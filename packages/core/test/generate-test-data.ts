...
const defaultRecordBatchChildren = () => [new Field('i32', new Int32()), new Field('f32', new Float32()), new Field('dict', new Dictionary(new Utf8(), new Int32()))];

const defaultStructChildren = () => [new Field('struct[0]', new Int32()), new Field('struct[1]', new Utf8()), new Field('struct[2]', new List(new Field('list[DateDay]', new DateDay())))];

const defaultMapChild = () => [new Field('', new Struct<{ key: Utf8; value: Float32 }>([new Field('key', new Utf8()), new Field('value', new Float32())]))][0];

const defaultUnionChildren = () => [new Field('union[0]', new Float64()), new Field('union[1]', new Dictionary(new Uint32(), new Int32())), new Field('union[2]', new Map_(defaultMapChild()))];
...
export const table = (lengths = [100], schema: Schema = new Schema(defaultRecordBatchChildren(), new Map([['foo', 'bar']]))): GeneratedTable => {
  ...
  const keys = memoize(() => keyBatches.reduce((keys: any[][], batch) => (!keys.length ? batch() : keys.map((idxs, i) => [...(idxs || []), ...(batch()[i] || [])])), []));
  const cols = memoize(() => colBatches.reduce((cols: any[][], batch) => (!cols.length ? batch() : cols.map((vals, i) => [...vals, ...batch()[i]])), []));
...
export const denseUnion = (length = 100, nullCount = (length * 0.2) | 0, children: Field[] = defaultUnionChildren()) =>
  vectorGenerator.visit(
    new DenseUnion(
      children.map((f) => f.typeId),
      children
    ),
    length,
    nullCount
  );
export const sparseUnion = (length = 100, nullCount = (length * 0.2) | 0, children: Field[] = defaultUnionChildren()) =>
  vectorGenerator.visit(
    new SparseUnion(
      children.map((f) => f.typeId),
      children
...
export const map = <TKey extends DataType = any, TValue extends DataType = any>(length = 100, nullCount = (length * 0.2) | 0, child: Field<Struct<{ key: TKey; value: TValue }>> = <any>defaultMapChild()) =>
...
function generateInt<T extends Int>(this: TestDataVectorGenerator, type: T, length = 100, nullCount = (length * 0.2) | 0): GeneratedVector<V<T>> {
  ...
  const values = memoize(() => {
    ...
    iterateBitmap(length, nullBitmap, (i, valid) => {
      values[i] = !valid ? null : stride === 1 ? data[i] : data.subarray(i * stride, (i + 1) * stride);
    });
    return values;
  });
...
function generateBinary<T extends Binary>(this: TestDataVectorGenerator, type: T, length = 100, nullCount = (length * 0.2) | 0): GeneratedVector<V<T>> {
  const nullBitmap = createBitmap(length, nullCount);
  const offsets = createVariableWidthOffsets(length, nullBitmap, undefined, undefined, nullCount != 0);
  const values = [...offsets.slice(1)].map((o, i) => (isValid(nullBitmap, i) ? o - offsets[i] : null)).map((length) => (length == null ? null : randomBytes(length)));
...
function generateDate<T extends Date_>(this: TestDataVectorGenerator, type: T, length = 100, nullCount = (length * 0.2) | 0): GeneratedVector<V<T>> {
  const values = [] as (number | null)[];
  const nullBitmap = createBitmap(length, nullCount);
  const data = type.unit === DateUnit.DAY ? createDate32(length, nullBitmap, values) : createDate64(length, nullBitmap, values);
  return {
...
function generateTimestamp<T extends Timestamp>(this: TestDataVectorGenerator, type: T, length = 100, nullCount = (length * 0.2) | 0): GeneratedVector<V<T>> {
  const values = [] as (number | null)[];
  const nullBitmap = createBitmap(length, nullCount);
  const multiple = type.unit === TimeUnit.NANOSECOND ? 1000000000 : type.unit === TimeUnit.MICROSECOND ? 1000000 : type.unit === TimeUnit.MILLISECOND ? 1000 : 1;
...
function generateTime<T extends Time>(this: TestDataVectorGenerator, type: T, length = 100, nullCount = (length * 0.2) | 0): GeneratedVector<V<T>> {
  ...
  const nullBitmap = createBitmap(length, nullCount);
  const multiple = type.unit === TimeUnit.NANOSECOND ? 1000000000 : type.unit === TimeUnit.MICROSECOND ? 1000000 : type.unit === TimeUnit.MILLISECOND ? 1000 : 1;
  const data = type.bitWidth === 32 ? createTime32(length, nullBitmap, multiple, values as (number | null)[]) : createTime64(length, nullBitmap, multiple, values as (Int32Array | null)[]);
...
function generateInterval<T extends Interval>(this: TestDataVectorGenerator, type: T, length = 100, nullCount = (length * 0.2) | 0): GeneratedVector<V<T>> {
  ...
  const values = memoize(() => {
    ...
    iterateBitmap(length, nullBitmap, (i: number, valid: boolean) => {
      values[i] = !valid ? null : stride === 2 ? new Int32Array(data.buffer, 4 * i * stride, stride) : new Int32Array([(data[i] / 12) | 0, data[i] % 12 | 0]);
    });
    return values;
  });
  iterateBitmap(length, nullBitmap, (i: number, valid: boolean) => {
...
function generateList<T extends List>(this: TestDataVectorGenerator, type: T, length = 100, nullCount = (length * 0.2) | 0, child = this.visit(type.children[0].type, length * 3, nullCount * 3)): GeneratedVector<V<T>> {
  ...
  const values = memoize(() => {
    ...
    const values: (T['valueType'] | null)[] = [...offsets.slice(1)].map((offset, i) => (isValid(nullBitmap, i) ? offset : null)).map((o, i) => (o == null ? null : childValues.slice(offsets[i], o)));
    return values;
  });
...
function generateMap<T extends Map_>(
  this: TestDataVectorGenerator,
  type: T,
  length = 100,
  nullCount = (length * 0.2) | 0,
  child = this.visit(type.children[0].type, length * 3, 0, [this.visit(type.children[0].type.children[0].type, length * 3, 0), this.visit(type.children[0].type.children[1].type, length * 3, nullCount * 3)])
): GeneratedVector<V<T>> {
...
type TypedArrayConstructor = typeof Int8Array | typeof Int16Array | typeof Int32Array | typeof Uint8Array | typeof Uint16Array | typeof Uint32Array | typeof Float32Array | typeof Float64Array;

const rand = Math.random.bind(Math);
const randomBytes = (length: number) => fillRandom(Uint8Array, length);
const randomString = (length: number) => {
  return randomstring(length);
}; //randomatic('?', length, { chars: `abcdefghijklmnopqrstuvwxyz0123456789_` });
...
const encodeUtf8 = ((encoder) => encoder.encode.bind(encoder) as (input?: string, options?: { stream?: boolean }) => Uint8Array)(new TextEncoder());
