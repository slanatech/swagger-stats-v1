...
export function validateTable({ keys, rows, cols, rowBatches, colBatches, keyBatches, table }: GeneratedTable) {
  return deferDescribe(
    `Table: ${table.schema}`,
    ([] as DeferredTest[]).concat(
...
export function validateRecordBatch({ rows, cols, keys, recordBatch }: GeneratedRecordBatch) {
  return deferDescribe(
    `RecordBatch: ${recordBatch.schema}`,
    ([] as DeferredTest[]).concat(
...
export function validateVector({ values: createTestValues, vector, keys }: GeneratedVector, sliced = false) {
    ...
    deferDescribe(`Validate ${vector.type} (sliced=${sliced})`, [
      ...
      deferTest(`gets expected values`, () => {
        ...
        try {
          ...
          throw new Error(`${vector}[${i}]: ${e}`);
        }
      }),
      (keys &&
        keys.length > 0 &&
        deferTest(`dictionary indices should match`, () => {
          expect.hasAssertions();
          ...
          try {
            while (++i < n) {
              indices.isValid(i) ? expect(indices.get(i)).toBe(keys[i]) : expect(indices.get(i)).toBeNull();
            }
          } catch (e) {
  ...
  if (!sliced) {
    const begin = (values.length * 0.25) | 0;
    const end = (values.length * 0.75) | 0;
    suites.push(
