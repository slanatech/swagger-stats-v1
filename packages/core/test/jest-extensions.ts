...
function format(jest: jest.MatcherUtils, actual: any, expected: any, msg = ' ') {
  return `${jest.utils.printReceived(actual)}${msg}${jest.utils.printExpected(expected)}`;
}
...
function toEqualTable(this: jest.MatcherUtils, actual: Table, expected: Table) {
  ...
  try {
    expect(actual.schema.metadata).toEqual(expected.schema.metadata);
  } catch (e) {
    failures.push(`${e}`);
  }
  (() => {
...
function toEqualVector<TActual extends Vector | [Vector | null, string?, string?], TExpected extends Vector | [Vector | null, string?]>(this: jest.MatcherUtils, actual: TActual, expected: TExpected) {
  let [v1, format1 = '', columnName = ''] = Array.isArray(actual) ? actual : [actual];
  let [v2, format2 = ''] = Array.isArray(expected) ? expected : [expected];

  if (v1 instanceof Column && columnName === '') {
    columnName = v1.name;
  }
  ...
  if (v1 == null || v2 == null) {
    return {
      pass: false,
      message: () => [[columnName, `(${format(this, format1, format2, ' !== ')})`].filter(Boolean).join(':'), `${v1 == null ? 'actual' : 'expected'} is null`].join('\n'),
    };
  }
  ...
  return {
    ...
    message: () => [[columnName, `(${format(this, format1, format2, ' !== ')})`].filter(Boolean).join(':'), ...allFailures.map(({ failures, title }) => (!failures.length ? `` : [`${title}:`, ...failures].join(`\n`)))].join('\n'),
  };
}
